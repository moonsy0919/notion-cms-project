...
## Phase 33: Admin 위저드 스텝 리스트 — 위치 필드와 정밀 정렬 ✅ 완료

> Phase 32에서 스텝 리스트를 아래로 내렸지만 참조 이미지 기준으로 아직 부족했습니다. 오른쪽 "기본 정보" 입력 폼의 마지막 필드인 "위치"까지 "기술 스택"의 설명 텍스트("사용하는 기술을 태그로 추가해주세요")가 내려오도록 Playwright로 실시간 측정하며 정밀 정렬했습니다.

- **Task 081: 브랜드 헤더 하단 여백을 위치 필드 기준으로 정밀 조정 (데스크톱)** ✅
  - `components/admin/ProfileWizard.tsx` — 브랜드 헤더 컨테이너 여백 `mb-20` → `mb-20 md:mb-[270px]`(모바일은 Phase 32 간격 유지, 데스크톱만 확대)
  - Playwright로 "사용하는 기술을 태그로 추가해주세요" `<p>`와 `#wizard-location` input의 `getBoundingClientRect()` 세로 중심 좌표를 비교하며 여백 값을 반복 조정 — `mb-[270px]`에서 두 요소의 세로 중심이 598px로 정확히 일치
...
**검증**: `npm run lint`·`npm run check`·`npm run build` 모두 통과. Playwright로 `/admin` 접속(1280px) — 화면 y=598px 지점에 빨간 기준선을 그려 "사용하는 기술을 태그로 추가해주세요" 텍스트와 "위치" 입력 필드가 정확히 같은 높이에서 겹침을 스크린샷으로 확인, "기본 정보" 폰트 크기(16px/14px)·중앙 정렬(0.5px 이내)·Continue 전환 회귀 없음 확인. 헤더 프로필 편집 Sheet 회귀 확인. 모바일(375px) 진행바가 Phase 32와 동일한 간격으로 유지됨을 스크린샷으로 확인, 콘솔 에러 0건 확인(가짜 API 키로 인한 예상된 Notion 400 로그 제외).
