 ...
 * @param baseDelayMs - 초기 대기 시간 ms (기본 1000ms, 이후 2배씩 증가)
 */
async function withRetry<T>(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (isRateLimit && hasRetriesLeft) {
        ...
        console.warn(
          `[AI] Rate limited. ${delayMs / 1000}초 후 재시도 (${attempt + 1}/${maxRetries})...`
        );
  ...
  // 루프가 항상 return 또는 throw로 종료되지만 TypeScript 컴파일러를 위해 유지
 ...
 * @returns Notion DB 프로퍼티 + 페이지 본문 블록 명세
 */
export async function analyzeRepo(data: GithubRepoData): Promise<AnalyzedRepoData> {
...
// 스모크 테스트: npx tsx --env-file=.env.local scripts/lib/ai-analyzer.ts --test [url]
if (process.argv[2] === "--test") {
  ...
  fetchGithubRepoData(testUrl)
    .then((repoData) => {
      console.log("[1/2] GitHub 데이터 수집 완료:", repoData.name);
      console.log("      언어:", Object.keys(repoData.languages).slice(0, 5).join(", "));
      console.log("[2/2] Claude 분석 시작...");
      return analyzeRepo(repoData);
    })
    .then((result) => {
      console.log("\n[분석 결과]\n");
      ...
      console.log(`blocks(${result.blocks.length}개):`);
      result.blocks.slice(0, 5).forEach((b) => {
        console.log(`  [${b.type}]`, b.text?.slice(0, 60) ?? "(divider)");
      });
      process.exit(0);
    })
    .catch((err) => {
